...
const createSemesterRegistrationIntoDB = async (
  ...
  if (checkUpcomingOrOngoing) {
    throw new AppError(
      ...
      `There is already a ${checkUpcomingOrOngoing?.status} semester`,
    );
  }
...
const updateSemesterRegistrationsIntoDB = async (
  ...
  const isSemesterRegisterExists =
    await getSingleSemesterRegistrationsFromDB(id);
  ...
  if (
    isSemesterRegisterExists?.status === semesterRegistrationStatuss.UPCOMING &&
    currentStatus === semesterRegistrationStatuss.ENDED
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      `Semester Registration Cannot update from ${isSemesterRegisterExists?.status} to ${currentStatus}`,
    );
  }

  if (
    isSemesterRegisterExists?.status === semesterRegistrationStatuss.ONGOING &&
    currentStatus === semesterRegistrationStatuss.UPCOMING
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      `Semester Registration Cannot update from ${isSemesterRegisterExists?.status} to ${currentStatus}`,
