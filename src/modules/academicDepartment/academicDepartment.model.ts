...
adademicDepartmentSchema.pre("save",async function(next){
  ...
  if(isExists){
   throw new AppError(httpStatus.NOT_FOUND,"This Academic Department is Alreay Created")
  }
  next()
})
...
adademicDepartmentSchema.pre("findOneAndUpdate",async function(next){
  ...
  if(!isExists){
    throw new AppError(httpStatus.NOT_FOUND,"There is no academic depertment with this id")
  }
  next()
})

export const academicDepartment = model<TAcademicDepartment>('AcademicDepartment',adademicDepartmentSchema);
